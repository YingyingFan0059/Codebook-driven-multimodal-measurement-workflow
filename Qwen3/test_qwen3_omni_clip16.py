...
"""
Qwen3-Omni-30B LoRA 测试脚本
方案：先裁剪固定时长音视频 clip，再进行推理
"""
...
EVAL_DIR = f"{PROJECT_ROOT}/outputs/qwen3_omni_eval_clip16"
CLIP_DIR = f"{EVAL_DIR}/clips_16s"
SWIFT_JSONL_PATH = f"{EVAL_DIR}/swift_eval_dataset_qwen3_clip16.jsonl"
INFER_RESULT_PATH = f"{EVAL_DIR}/infer_result_qwen3_clip16.jsonl"
...
# 如需固定特定 checkpoint，可手动填绝对路径；否则自动取最新
LORA_PATH = None
...
def build_video_path_index(base_dir: str) -> dict:
    path_map = {}
...
def make_clip_ffmpeg(src_video: str, dst_video: str, seconds: int = 16) -> bool:
    """
    裁剪前 N 秒，保留视频+音频。
    -c:v libx264 / -c:a aac 更稳，但会转码，稍慢
    """
    ...
    try:
        ...
        return result.returncode == 0 and os.path.exists(dst_video) and os.path.getsize(dst_video) > 0
    except Exception:
        return False


def prepare_swift_dataset_with_clips(
    ...
    for _, row in df_gold.iterrows():
        ...
        dst_video = os.path.join(clip_dir, f"{vid}_clip{clip_seconds}s.mp4")
        ok = make_clip_ffmpeg(src_video, dst_video, seconds=clip_seconds)
        if not ok:
            clip_failed.append(vid)
            continue
    ...
    log_print(f"[INFO] 缺失原视频数: {len(missing_videos)}")
    log_print(f"[INFO] clip 失败数: {len(clip_failed)}")

    if missing_videos:
        log_print(f"[WARN] 前10个缺失原视频 video_id: {missing_videos[:10]}")
    if clip_failed:
        log_print(f"[WARN] 前10个 clip 失败 video_id: {clip_failed[:10]}")

    return jsonl_path, len(valid_records), len(clip_failed)
...
def run_swift_inference():
    ...
    log_print("=" * 80)
    log_print("[INFO] Qwen3-Omni clip16 测试开始")
    log_print(f"[INFO] 使用 checkpoint: {lora_path}")
    log_print(f"[INFO] CLIP_SECONDS: {CLIP_SECONDS}")
    log_print(f"[INFO] EVAL_DIR: {EVAL_DIR}")
    log_print("=" * 80)

    log_print("[INFO] 正在建立视频路径索引...")
    global_video_paths = build_video_path_index(VIDEO_BASE_DIR)
    log_print(f"[INFO] 视频索引数: {len(global_video_paths)}")

    jsonl_path, valid_count, clip_failed_count = prepare_swift_dataset_with_clips(
        df_gold=df_gold,
        video_paths=global_video_paths,
    ...
    pred_csv_path = os.path.join(EVAL_DIR, "eval_qwen3_clip16_predictions.csv")
    ...
    report_text.append("Qwen3-Omni clip16 测试报告")
    report_text.append("=" * 80)
    ...
    report_text.append(f"Clip failed rows: {clip_failed_count}")
    ...
    report_path = os.path.join(EVAL_DIR, "eval_qwen3_clip16_report.txt")
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(report_text)

    cm_path = os.path.join(EVAL_DIR, "eval_qwen3_clip16_confusion_matrix.csv")
    pd.DataFrame(
        cm,
    ...
    pred_dist_path = os.path.join(EVAL_DIR, "eval_qwen3_clip16_pred_distribution.json")

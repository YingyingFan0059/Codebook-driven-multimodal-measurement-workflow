...
# 关键修改：复现结果统一写到新的文件夹，不与原结果混放
EVAL_OUT_DIR = os.environ.get("QWEN2_EVAL_OUT_DIR", str(Path(__file__).resolve().parents[2] / "artifacts" / "qwen2" / "outputs" / "scaling_eval_repro"))
...
def evaluate_model(train_size, test_csv_name="test_main.csv"):
    print(f"\n{'='*50}\n🔍 正在统考复现: lora_5class_{train_size} | 考卷: {test_csv_name}\n{'='*50}", flush=True)

    lora_path = os.path.join(RUNS_DIR, f"lora_5class_{train_size}")
    ...
    if len(y_true) > 0:
        ...
        print(f"📝 Predictions saved to: {pred_csv_path}", flush=True)
        print(f"📝 Report saved to: {report_txt_path}", flush=True)
        print("=" * 40 + "\n", flush=True)
    else:
...
if __name__ == "__main__":
    ...
    print("🔔 启动全量 Scaling Law 复现评估任务...", flush=True)
    for size in scaling_sizes:
        evaluate_model(size, test_csv_name="test_main.csv")
    print("\n🎉 Scaling Law 复现实验评估已全部完成。", flush=True)
